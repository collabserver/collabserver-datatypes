...
class GGraph {
   ...
   private:
    struct _Vertex {
       public:
        ...
        friend bool operator!=(const _Vertex& lhs, const _Vertex& rhs) { return !(lhs == rhs); }

        friend bool operator<(const _Vertex& lhs, const _Vertex& rhs) { return lhs._content < rhs._content; }
   ...
   public:
    void addVertex(const K& id, const T& value) { _adj.insert({id, {id, value, {}}}); }
   ...
   public:
    friend bool operator==(const GGraph& lhs, const GGraph& rhs) { return lhs._adj == rhs._adj; }

    friend bool operator!=(const GGraph& lhs, const GGraph& rhs) { return !(lhs == rhs); }

    friend std::ostream& operator<<(std::ostream& out, const GGraph<K, T>& o) {
        out << "GGraph = {\n";
